...
const correctAppearance = parseInt(correctCharacter.firstAppearance.match(/\d+/)[0]); // Extract the number from the firstAppearance string
const correctHeight = parseInt(correctCharacter.height.match(/\d+/)[0]); // Extract the number from the height string
const correctWeight = parseInt(correctCharacter.weight.match(/\d+/)[0]); // Extract the number from the weight string
...
inputBox.addEventListener('input', function() {
    ...
    charactersList.innerHTML = ''; // Clear previous suggestions
...
inputBox.addEventListener('keydown', function(event) {
    if (event.key === 'Enter') {
        ...
        // Show the table headers when a character is entered
        if (charactersTableHead.classList.contains('hidden')) {
            charactersTableHead.classList.remove('hidden');
        }

        // Filter the character that exactly matches the input
        ...
        if (matchedCharacter) {
            ...
            const matchedAppearance = parseInt(firstAppearanceYearMatch[0]); // Extract the number from the firstAppearance string
            const matchedHeight = parseInt(heightMatch[0]); // Extract the number from the height string
            const matchedWeight = parseInt(weightMatch[0]); // Extract the number from the weight string
            ...
            // Check if the matched character is the correct character
